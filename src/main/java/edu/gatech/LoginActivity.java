...
public class LoginActivity extends Activity {
	...
	public class UserLoginTask extends AsyncTask<Void, Void, Boolean> {
		private DBHandler database = new DBHandler(LoginActivity.this);
		@Override
		protected Boolean doInBackground(Void... params) {
			Cursor c = database.selectUser(mEmail, mPassword);
			c.moveToFirst();
			if (((c != null) && (c.getCount() > 0))){
				if (!c.getString(0).equals(mEmail) || !c.getString(1).equals(mPassword)) {
					return false;
				}
				return true;
			}
